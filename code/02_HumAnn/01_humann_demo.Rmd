...
```{r}
pathway %>% 
  import_humann_pathway_mia(rm_un = FALSE, # Remove UNMAPPED and UNINTEGRATED Pathways
...
We can also import Gene family data:

```{r}
kegg %>% 
  import_humann_pathway_mia(rm_un = TRUE, 
                            return_unstrat = FALSE,
                            add_CHOCOPhlAn_taxonomy = TRUE, 
...
kegg_mia %>% 
  ...
  head()
```
Let's focus on the unstratified data as a start:
...
pw_mia %>% 
  ...
  transform_sample_counts(function(x) x/sum(x) * 100) %>% # renorm or not??
  phyloseq_alphas() %>% 
